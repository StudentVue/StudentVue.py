...
class StudentVue:
    ...
    def getGradingInfobyPeriod(self, period):
        ...
        for row in raw_data[1:]:
            ...
            for column in row:
                if raw_data[0][row.index(column)] == 'Points Possible':
                    this_assignment_data[raw_data[0]
                                         [row.index(column)]] = float(column)
                elif raw_data[0][row.index(column)] == 'Points':
                    this_assignment_data[raw_data[0]
                                         [row.index(column)]] = float(column)
                elif raw_data[0][row.index(column)] == 'Weight':
                    this_assignment_data[raw_data[0][row.index(column)
                                                     ]] = float(column.strip('%')) / 100
                elif raw_data[0][row.index(column)] == 'Average':
                    this_assignment_data[raw_data[0][row.index(column)
                                                     ]] = float(column.strip('%')) / 100
                else:
                    this_assignment_data[raw_data[0]
